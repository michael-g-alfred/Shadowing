...
final class UserRepository: UserRepositoryProtocol {
    ...
    private var inFlightSummaryRequests: [String: Task<UserSummaryModel, Error>] = [:]
        ...
        /// Observer token for ``Notification/Name/authDidSignOut``, retained so
        /// the observation is removed when this repository is deallocated.
        ///
        /// Marked `nonisolated(unsafe)` deliberately: `deinit` is always
        /// `nonisolated`, even on a `@MainActor` class, so it cannot touch a
        /// MainActor-isolated property directly. `NotificationCenter.removeObserver`
        /// is thread-safe, so reading/clearing this from `deinit` is safe even
        /// though it bypasses actor isolation. All other access happens on the
        /// main actor, from `init`.
    private nonisolated(unsafe) var signOutObserver: NSObjectProtocol?
    
    init(network: MGNetworkServiceProtocol, authRepository: AuthRepositoryProtocol) {
        self.network = network
        self.authRepository = authRepository
        
            // Clear the summary cache on sign-out so a subsequent sign-in (as
            // the same or a different user) never serves stale cached summaries.
    ...
    func fetchUserSummary(id: String) async throws -> UserSummaryModel {
        ...
        if let cached = summaryCache[id] {
            ...
            //
            // Because this class is @MainActor, everything between "check
            // inFlightSummaryRequests" and "register the new task" below runs
            // without interruption — there's no `await` in between, so no other
            // caller can slip in and start a second Task for the same id. The
            // task is registered synchronously *before* any awaiting begins.
        if let existingTask = inFlightSummaryRequests[id] {
            return try await existingTask.value
        }
        
        let task = Task<UserSummaryModel, Error> { [weak self] in
            guard let self else { throw AuthError.noSession }
            let accessToken = try await self.getValidToken()
            let config = APIConfig.userSummary(id: id, accessToken: accessToken)
            let response: APIResponseDTO<UserSummaryResponseDTO> = try await self.network.request(config)
            return response.data.user.toDomain()
        }
            // Registered immediately, synchronously, before any suspension point
            // — closes the race where a rapid sequential call could slip in
            // between task creation and registration and start a duplicate fetch.
        inFlightSummaryRequests[id] = task
        
            // Always clear the in-flight marker once this fetch settles
            // (success or failure), so the next call for this id can retry.
        defer { inFlightSummaryRequests[id] = nil }
